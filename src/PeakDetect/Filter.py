...
# -*- coding: utf-8 -*-
'''
Date: 2022-10-17 16:08:27
LastEditors: gakkispy && yaosenjun@cii.com
LastEditTime: 2022-10-18 19:58:08
FilePath: /overlap_project/src/PeakDetect/Filter.py
'''
...
def savgol_filter(data, window_size=5, order=2):
    return signal.savgol_filter(data, window_size, order)

# wavelet finter

def wavelet_filter(new_df):
    data = new_df
...
def uv_filter(data, layer=200):
    _uv_filter = Denoiser()
    return _uv_filter.denoise(data, layer)
...
def get_filters(filter_name):
    filter_list =  {
        ...
        'conv': conv_filter,
        'uv': uv_filter,
        'medfit': signal.medfilt,
        'wiener': signal.wiener,
    }
    return filter_list[filter_name]

def data_with_filted(filter=savgol_filter, data=None):
    return filter(data), filter.__name__